...
<p><strong>Your phone is a poor light meter if it has no cosine correction</strong> (highly likely does not). I can set up conditions where my Samsung A51 (and Samsung S7) are ten times off a true reading. This is a hardware limitation that can not be corrected with an app. Phones are basically worthless for color LEDs due to the luminous efficiency issue. Based on hands-on experience, I automatically discount all lux measurements done with phones.</p>"
